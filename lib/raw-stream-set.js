module.exports = class RawStreamSet {
  ...
  add (opts) {
    ...
    function onclose () {
      const has = self._streams.has(id & self._prefix)
      if (!has) console.log('raw-stream-set onclose does not have it', self._streams.size)
  ...
  async destroy () {
    console.log('raw-stream-set destroy()')
    const destroying = []
    let count = 0
    ...
    for (const stream of this._streams.values()) {
      destroying.push(new Promise((resolve) => stream
        .once('close', () => {
          console.log('raw stream destroyed and resolved', ++count)
          resolve()
        })
        .destroy()
      ))
    ...
    console.log('raw-stream-set destroying', destroying.length)
    await Promise.allSettled(destroying)
    console.log('raw-stream-set destroyed')
  }
}
