...
module.exports = class Nat {
  ...
  get type () {
    ...
    // TODO: if a !== b !== c also return 2 (random)
  ...
  autoSample (retry = true) {
    ...
    for (let node = this.dht.nodes.latest; node && this._sampler.size + pending < this._minSamples; node = node.prev) {
      if (skip > 0) {
        skip--
        continue
      }
    ...
    function onskip () {
      if (--pending === 0 && self._sampler.size < self._minSamples) {
        if (retry) self.autoSample(false)
        else self._resolve()
      }
    }
  }
