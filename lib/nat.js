...
module.exports = class Nat {
  ...
  autoSample(retry = true) {
    ...
    for (let node = this.dht.nodes.latest; node; node = node.prev) {
    ...
    if (availableNodes.length > maxPings && this.dht.sortNodesByRTT) {
      // Use RTT-based selection if available
      selectedNodes = this.dht.sortNodesByRTT(availableNodes).slice(0, maxPings * 2)
    } else {
      // Fallback: skip some nodes if we have many
    ...
    for (const node of selectedNodes) {
      if (this.sampled + pending >= maxPings) break
      ...
      this.session.ping(node, { socket, retry: false }).then((res) => {
        // Track RTT
