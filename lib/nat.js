...
module.exports = class Nat {
  ...
  get firewall () {
    if (this.frozen) return this._firewall
    if (this.dht.firewalled === false) return FIREWALL.OPEN

    if (this._sampler.size >= this._minSamples) {
      return this._sampler.port
        ? FIREWALL.CONSISTENT
        : (this._sampler.host ? FIREWALL.RANDOM : FIREWALL.UNKNOWN)
    }

    if (this._sampler.size === 3 && this._sampler.port) {
      return FIREWALL.CONSISTENT
    }
  ...
  get address () {
    if (this.frozen) return this._address
