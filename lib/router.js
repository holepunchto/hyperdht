...
module.exports = class HolepunchRouter {
  ...
  async onconnect (req) {
    ...
    const isServer = !!(state && state.onconnect)
    const relay = state && state.relay

    if (isServer) {
      let reply = null
      try {
        reply = noise && await state.onconnect({ noise, peerAddress }, req)
      } catch (e) {
        safetyCatch(e)
        return
      }
  ...
  async onholepunch (req) {
    ...
    const state = req.target && this.forwards.get(req.target.toString('hex'))
    const isServer = !!(state && state.onholepunch)
    const relay = state && state.relay
    ...
    switch (mode) {
      ...
      case FROM_RELAY: {
        ...
        let reply = null
        try {
          reply = await state.onholepunch({ id, payload, peerAddress }, req)
        } catch (e) {
          safetyCatch(e)
          return
        }
