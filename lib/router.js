...
console.log('Using debug DHT build')
...
module.exports = class HolepunchRouter {
  ...
  async onpeerhandshake (req) {
    console.log('onpeerhandshake [1] incoming')
    ...
    console.log('onpeerhandshake [2] decoded', hs, 'isServer', isServer)

    if (isServer) {
      let reply = null
      try {
        console.log('onpeerhandshake [3] prehandshake')
        reply = noise && await state.onpeerhandshake({ noise, peerAddress }, req)
        console.log('onpeerhandshake [4] posthandshake')
      } catch (e) {
        console.log('onpeerhandshake [5] handshake error', e)
        safetyCatch(e)
        return
      }
