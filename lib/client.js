...
module.exports = class Client {
  ...
  async connectThrough (node) {
    ...
    const rawSocket = await this.punch.connected
    return {
      rawSocket
    }
  }
  ...
  async _roundOne (serverAddress, serverRelay, retry) {
    ...
    let reroll = 0
    while (reroll++ < MAX_REROLLS && this.punch.remoteNat >= 2 && this.punch.nat.type >= 2) {
      console.log('bad looking socket... try rerolling it')
      this.punch.reset()
      await this.punch.nat.analyzing
      if (this._done()) return null
      console.log('after reroll... nat=', this.punch.nat.type)
      if (this.punch.nat.type === 1) return this._roundOne(serverAddress, serverRelay, false)
    }
