...
module.exports = class Client {
  ...
  async _roundOne (serverAddress, serverRelay, retry) {
    ...
    // This currently HAS to come after session opening as we otherwise run into all kinds of
    // "message is dropped due to low ttl" because of the stateful setTTL api :/
    this.punch.nat.autoSample()
    ...
    if (this.punch.remoteNat === 0 || this.punch.nat.type === 0) {
      await this._abort(serverAddress, serverRelay)
      throw new Error('Holepunching probe did not finish in time')
    }
  ...
  _abort (addr, relay) {
    return this._holepunch(addr, relay, {
      status: 2,
      nat: 0,
      address: null,
      remoteAddress: null,
