...
module.exports = class HolepunchRouter {
  ...
  async connect (target, { noise, relayAddress }, to) {
    const dht = this.dht
    ...
    return {
      ...
      serverRelayAddress: con.peerAddress ? con.relayAddress || res.from : null,
      clientAddress: res.to,
      clientRelayAddress: con.peerAddress ? to : null
    }
  }
  ...
  async onconnect (req) {
    ...
    if (isServer) {
      let reply = null
      try {
        reply = noise && await state.server.onconnect(noise, req)
      } catch (e) {
        safetyCatch(e)
        return
      }
      ...
      switch (mode) {
        case FROM_CLIENT: {
          req.reply(c.encode(connect, { mode: REPLY, noise: reply.noise, peerAddress: null, relayAddress: null }), opts)
          return
        }
        case FROM_RELAY: {
          req.relay(c.encode(connect, { mode: FROM_SERVER, noise: reply.noise, peerAddress, relayAddress: null }), req.from, opts)
          return
        }
        case FROM_SECOND_RELAY: {
          if (!relayAddress) return
          req.relay(c.encode(connect, { mode: FROM_SERVER, noise: reply.noise, peerAddress, relayAddress: req.from }), relayAddress, opts)
          return
        }
      }
    } else {
      switch (mode) {
        case FROM_CLIENT: {
          ...
          req.relay(c.encode(connect, { mode: FROM_RELAY, noise, peerAddress: req.from, relayAddress: null }), relay || relayAddress)
          return
        }
        case FROM_RELAY: {
        ...
        case FROM_SERVER: {
          ...
          req.reply(c.encode(connect, { mode: REPLY, noise, peerAddress: req.from, relayAddress }), { to: peerAddress, closerNodes: false, token: false })
          return
        }
      }
    }
