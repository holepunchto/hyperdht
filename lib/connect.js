...
async function connectAndHolepunch (c, opts) {
  const route = c.dht._connectionPool.get(c.remotePublicKey)
...
console.log('post', serverAddress, clientAddress, relayed)
  ...
  c.onsocket = function (socket, port, host) {
    ...
    c.dht._connectionPool.add(c.remotePublicKey, rawStream, socket)
