...
async function findAndConnect(c, opts) {
  ...
  if (!closestNodes) {
    const cachedRelayAddreses = c.dht._relayAddressesCache.get(c.id)
    if (cachedRelayAddreses) closestNodes = cachedRelayAddreses
  }
  ...
  c.encryptedSocket.relayAddresses = []
  ...
  try {
    for (let i = 0; i < tries && !isDone(c) && !c.connect; i++) {
      ...
      for await (const data of c.query) {
        ...
        c.encryptedSocket.relayAddresses.push(data.from)
        attempts++
...
async function connectThroughNode(c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    ...
    if (c.rawStream.connected) {
      ...
      if (c.encryptedSocket.relayAddresses.length) {
        c.dht._relayAddressesCache.set(c.id, c.encryptedSocket.relayAddresses)
      }
    }
