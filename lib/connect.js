...
module.exports = function connect (dht, publicKey, opts = {}) {
  ...
  const c = {
    ...
    invalidateStream: opts.invalidateStream || null
  }
...
async function connectThroughNode (c, address, socket) {
  if (!c.requesting) {
    ...
    c.requesting = true
    console.log('sending handshake (client)')
    c.request = await c.handshake.send({
      ...
      relayThrough: c.relayThrough
        ...
        : null,
      invalidateStream: c.invalidateStream
    })
    console.log('received handshake reply (client)')
    if (isDone(c)) return
  }
