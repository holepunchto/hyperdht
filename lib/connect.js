...
function isDone(c) {
  ...
  if (
    c.relaySocket &&
    !!(c.puncher && !c.puncher.connected && !c.puncher.destroyed)
  ) {
    return false
  }
  // we are done
...
async function holepunch(c, opts) {
  ...
  if (c.lan && relayed && clientAddress.host === serverAddress.host) {
    ...
    if (serverAddresses.length > 0) {
      ...
      const addr =
        Holepuncher.matchAddress(myAddresses, serverAddresses) ||
        serverAddresses[0]
...
async function findAndConnect(c, opts) {
  let attempts = 0
  let closestNodes =
    opts.relayAddresses && opts.relayAddresses.length
      ? opts.relayAddresses
      : null
  ...
  if (!c.connect) {
    maybeDestroyEncryptedSocket(
      c,
      attempts ? PEER_CONNECTION_FAILED() : PEER_NOT_FOUND()
    )
  }
}
...
async function connectThroughNode(c, address, socket) {
  if (!c.requesting) {
    ...
    const localAddrs = c.lan
      ? Holepuncher.localAddresses(c.dht.io.serverSocket)
      : null
    const addresses4 = []
  ...
  c.onsocket = function (socket, port, host) {
    ...
    if (c.rawStream.connected) {
      const remoteChanging = c.rawStream.changeRemote(
        socket,
        c.connect.payload.udx.id,
        port,
        host
      )
...
function relayConnection(c, relayThrough, payload, hs) {
  ...
  c.relayClient = relay.Client.from(c.relaySocket, {
    id: c.relaySocket.publicKey
  })
