...
async function connectAndHolepunch (c, opts) {
  console.log('connectAndHolepunch')
  const route = c.reusableSocket ? c.dht._socketPool.routes.get(c.remotePublicKey) : null

  if (route) {
    console.log('connectAndHolepunch -> retryRoute')
    await retryRoute(c, route)
    if (isDone(c)) return
  }

  console.log('connectAndHolepunch -> findAndConnect')
  ...
  console.log('connectAndHolepunch -> holepunch')
...
async function holepunch (c, opts) {
  ...
  let remoteHolepunchable = !!(payload.holepunch && payload.holepunch.relays.length)
  // remoteHolepunchable = false

  const relayed = diffAddress(serverAddress, relayAddress)

  console.log('holepunch', { relayAddress, serverAddress, clientAddress }, payload, { remoteHolepunchable, relayed, opts })
  console.log('firewall open?', payload.firewall === FIREWALL.OPEN)
  ...
  if (payload.firewall === FIREWALL.OPEN || (relayed && !remoteHolepunchable)) {
    ...
    for (const addr of addrs) {
      if (isPrivate(addr.host)) {
        console.log('would have skipped addr', addr.host)
        // continue
      }
  ...
  if (relayed && clientAddress.host === serverAddress.host) {
    const allowsLocalConnections = opts.localConnection !== false
    console.log('second strategy', { allowsLocalConnections })

    if (allowsLocalConnections) {
      const addrs = [...payload.addresses6, ...payload.addresses4]

      for (const addr of addrs) {
        if (!isPrivate(addr.host)) {
          console.log('skip addr', addr.host)
          continue
        }

        // addr.port = 49737

        console.log('holepunching local network', addr)
        ...
        await c.puncher.openSession({ host: addr.host, port: addr.port })
        await c.puncher.openSession({ host: addr.host, port: addr.port }, c.dht.socket)
        await c.puncher.openSession({ host: addr.host, port: addr.port }, c.dht.io.clientSocket)
        await c.puncher.openSession({ host: addr.host, port: addr.port }, c.dht.io.serverSocket)
        await c.puncher.openSession({ host: addr.host, port: 49737 })
        await c.puncher.openSession({ host: addr.host, port: 49737 }, c.dht.socket)
        await c.puncher.openSession({ host: addr.host, port: 49737 }, c.dht.io.clientSocket)
        await c.puncher.openSession({ host: addr.host, port: 49737 }, c.dht.io.serverSocket)
        console.log('after opening session')
        ...
        // serverRelay.peerAddress = addr

        // beginHolepunching(c, opts, serverAddress, serverRelay, relayAddress, addr)
  ...
  console.log('payload.holepunch.relays', payload.holepunch.relays)
  console.log('relayAddress', relayAddress)

  const serverRelay = pickServerRelay(payload.holepunch.relays, relayAddress)

  // serverRelay.peerAddress = payload.addresses4[0]
  // serverAddress = payload.addresses4[0]

  console.log('serverRelay', serverRelay)

  // Begin holepunching!

  beginHolepunching(c, opts, serverAddress, serverRelay, relayAddress)
}

async function beginHolepunching (c, opts, serverAddress, serverRelay, relayAddress, localAddress) {
  console.log('beginHolepunching', { opts, serverAddress, serverRelay, relayAddress })

  let probe
  try {
    console.log('first probe', { serverAddress: (opts.fastOpen === false ? null : serverAddress), serverRelay })
    probe = await probeRound(c, opts.fastOpen === false ? null : serverAddress, serverRelay, true, localAddress)
    console.log({ probe })
  } catch (err) {
  ...
  console.log('probe result afterwards', { token, peerAddress })
  ...
  if (!diffAddress(serverRelay.relayAddress, relayAddress) && diffAddress(serverAddress, peerAddress)) {
    console.log('try open session')
    const prevServerAddress = serverAddress
    serverAddress = peerAddress
    await c.puncher.openSession(serverAddress)
    if (isDone(c)) return
    // serverAddress = prevServerAddress // + at least for local network holepunching
  }
  ...
  try {
    console.log('round pounch', { serverAddress, relayAddress })
    await roundPunch(c, serverAddress, token, relayAddress, localAddress)
    console.log('after round pounch')
  } catch (err) {
    console.log('round pounch error', err)
    if (isDone(c)) return
...
async function connectThroughNode (c, address, socket) {
  ...
  console.log('connectThroughNode connect property', Object.assign({}, c.connect, { payload: '---' })) // c.connect.payload
...
async function probeRound (c, serverAddress, serverRelay, retry, localAddress) {
...
async function roundPunch (c, serverAddress, remoteToken, clientRelay, localAddress) {
  console.log('roundPunch', { serverAddress })
