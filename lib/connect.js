...
async function holepunch (c, opts) {
  ...
  if (c.firewalled && payload.network === OPEN) {
    // Remote will connect to us, do nothing as we'll get a connection or timeout
    return
  }

  if (!relayed || !payload.holepunch || !payload.holepunch.relays.length || payload.network === OPEN) {
...
async function connectThroughNode (c, node) {
  if (!c.request) {
    ...
    c.request = c.handshake.send({
      error: 0,
      network: c.firewalled ? UNKNOWN : OPEN,
      protocols: TCP | UTP,
      holepunch: null,
      addresses: addr ? [addr] : []
    })
...
async function updateHolepunch (c, peerAddress, relayAddr, payload) {
  ...
  const { error, network, punching, address, remoteToken } = remotePayload
  if (error > 0) {
  ...
  if (c.pair.remoteNat === 0 && network !== 0 && address && (c.pair.remoteNat !== 2 || address.port !== 0)) {
    c.pair.remoteNat = network
...
async function roundOne (c, serverAddress, serverRelay, retry) {
  ...
  if (c.pair.remoteNat === 0 || c.pair.nat.type === 0) {
