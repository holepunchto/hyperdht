...
module.exports = function connect(dht, publicKey, opts = {}) {
  ...
  // Run in the background and forward failures to the returned stream
  if (dht.suspended) encryptedSocket.destroy(SUSPENDED())
  else connectAndHolepunch(c, opts).catch(onConnectError)
  ...
  function onConnectError(err) {
    if (c.puncher) c.puncher.onabort = noop
    destroyPuncher(c)
    maybeDestroyEncryptedSocket(c, err)
  }
