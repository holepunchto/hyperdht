...
async function connectAndHolepunch (c, opts) {
  ...
  c.dht.tracer.trace('findAndConnect')
  await findAndConnect(c, opts)
  c.dht.tracer.trace(`isDone ${isDone(c)}`)
  if (isDone(c)) return
  ...
  c.dht.tracer.trace('holepunch')
  await holepunch(c, opts)
  c.dht.tracer.trace('holepunch done')
}
...
async function holepunch (c, opts) {
  ...
  console.log('remoteHolepunchable', remoteHolepunchable)
  ...
  console.log('c.puncher 1', !!c.puncher)
...
async function localLanStrategy (c, opts) {
  const { payload } = c.connect
...
async function connectThroughNode (c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    c.dht.tracer.trace('socket', { port, host })
    if (c.rawStream === null) {
      c.dht.tracer.trace('already-punched')
      return // Already hole punched
    }

    if (c.rawStream.connected) {
      c.dht.tracer.trace('switching-sockets')
    ...
    console.log('puncher', !!c.puncher)
    if (c.puncher) {
      c.dht.tracer.trace('killing-puncher')
