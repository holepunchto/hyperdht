...
// Parallel probing: sends multiple probe attempts with staggered timing
// First successful probe wins, others are ignored
async function parallelProbe(target, probeFn, avgRTT, maxProbes = 3) {
  const delays = avgRTT > 0 ? [0, Math.max(50, avgRTT >> 1), Math.max(100, avgRTT)] : [0, 100, 250]

  // Use Promise.race-style behavior: return on first success, throw if all fail
  const probes = delays
    .slice(0, maxProbes)
    .map((delay) =>
      delay > 0
        ? new Promise((r) => setTimeout(r, delay)).then(() => probeFn(target))
        : probeFn(target)
    )
  ...
  return new Promise((resolve, reject) => {
    ...
    probes.forEach((probe, i) => {
      probe.then(
        ...
        (error) => {
          errors[i] = error
          if (++settled === probes.length && !resolved) {
            reject(errors[0] || new Error('All parallel probes failed'))
          }
        }
      )
    })
...
async function connectAndHolepunch(c, opts) {
  ...
  // Phase 7.1: Try direct connection first (if available) - with timeout
  if (
    (await Promise.race([
      tryDirectConnection(c, opts).catch(() => false),
  ...
  // Start discovery and try early handshake with first discovered node
  await findAndConnectParallel(c, opts)
  if (isDone(c)) return
...
async function holepunch(c, opts) {
  ...
  if (c.firewall === FIREWALL.OPEN) {
    c.passiveConnectTimeout = setTimeout(onabort, c.dht.getRTTBasedTimeout?.(10000) ?? 10000)
    return
  }
...
async function tryDirectConnection(c, opts) {
  const key = c.target.toString('hex')
  const addresses = []

  const cached = c.dht._directConnectionCache?.get(key)
  if (cached && Date.now() - cached.timestamp < 300000) {
    addresses.push(cached.address)
  }
  ...
  for (const addr of addresses) {
    try {
      await Promise.race([
        connectThroughNode(c, addr, null),
        new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 2000))
      ])
      if (c.connect) {
        c.dht._directConnectionCache?.set(key, { address: addr, timestamp: Date.now() })
        return true
      }
    } catch {
      if (addr === cached?.address) c.dht._directConnectionCache?.delete(key)
    }
  }
...
async function findAndConnectParallel(c, opts) {
  let attempts = 0
  ...
  if (!closestNodes && c.dht._connectionCache) {
    const cached = c.dht._connectionCache.get(c.target.toString('hex'))
    if (cached && Date.now() - cached.timestamp < 60000) {
      closestNodes = cached.path
    }
  }
  ...
  // If we have closestNodes, try handshake immediately in parallel with discovery
  let earlyHandshakePromise = null
  if (closestNodes?.length) {
    const sorted = c.dht.sortNodesByRTT?.(closestNodes) ?? closestNodes
    earlyHandshakePromise = connectThroughNode(c, sorted[0], null).catch(() => null)
  }
  ...
  try {
    earlyHandshakePromise?.catch(() => {})

    for (let i = 0; i < tries && !isDone(c) && !c.connect; i++) {
      const useRTTBased = closestNodes !== null && c.dht._nodeRTT && c.dht.getAverageRTT() != null
      c.query = c.dht.findPeer(c.target, {
        hash: false,
        session: c.session,
        closestNodes,
        onlyClosestNodes: closestNodes !== null,
        retries: closestNodes ? 1 : useRTTBased ? 2 : 3
      })
...
async function findAndConnect(c, opts) {
  return findAndConnectParallel(c, opts)
}
...
async function connectThroughNode(c, address, socket) {
  ...
  if (address) {
    ...
    // Always cache relay path for discovery
    const relayAddr = { host: address.host, port: address.port }
    c.dht._connectionCache?.set(key, { path: [relayAddr], timestamp })

    // Also cache peer's best reachable address for direct connection
    const peerAddr = getFirstRemoteAddress(payload.addresses4, serverAddress)
    if (peerAddr) {
      c.dht._directConnectionCache?.set(key, {
        address: { host: peerAddr.host, port: peerAddr.port },
        timestamp
      })
    }
  }
...
async function probeRound(c, serverAddress, serverRelay, retry) {
  const avgRTT = c.dht.getAverageRTT?.() ?? null

  // Helper to probe with or without parallelization
  const probe = async (addr) => {
    if (!addr) return
    return c.dht.parallelProbing !== false && avgRTT != null
      ? parallelProbe(addr, (a) => c.puncher.openSession(a), avgRTT, 3)
      : c.puncher.openSession(addr)
  }

  // Open a quick low ttl session against what we think is the server
  await probe(serverAddress)
  if (isDone(c)) return null
  ...
  // Reduced delay for faster connections (500ms instead of 1000ms)
...
function relayConnection(c, relayThrough, payload, hs) {
  ...
  c.relayTimeout = setTimeout(onabort, c.dht.getRTTBasedTimeout?.(15000) ?? 15000, null)
