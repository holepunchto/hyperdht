...
module.exports = function connect (dht, publicKey, opts = {}) {
  ...
  c.rawStream.once('connect', () => {
    dht.tracer.trace('stream-open', {
      stream: {
        publicKey: encryptedSocket.publicKey,
        remotePublicKey: encryptedSocket.remotePublicKey,
        isInitiator: encryptedSocket.isInitiator
      }
    })
  ...
  encryptedSocket.on('close', function () {
    dht.tracer.trace('stream-close', {
      error: getStreamError(encryptedSocket),
      stream: {
        publicKey: encryptedSocket.publicKey,
        remotePublicKey: encryptedSocket.remotePublicKey,
        isInitiator: encryptedSocket.isInitiator
