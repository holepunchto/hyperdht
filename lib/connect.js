...
module.exports = function connect (dht, publicKey, opts = {}) {
  ...
  encryptedSocket.on('open', function () {
    if (!c.relaySocket || !c.puncher || c.puncher.connected || c.puncher.destroyed) c.session.destroy()
  })
...
async function holepunch (c, opts) {
  ...
  const onabort = () => {
    if (c.relayToken === null) maybeDestroyEncryptedSocket(c, HOLEPUNCH_ABORTED())
  }
...
async function connectThroughNode (c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    ...
    if (c.relaySocket && c.puncher && c.puncher.connected) {
      c.session.destroy()
    }
...
async function roundPunch (c, serverAddress, remoteToken, clientRelay) {
  ...
  if (c.puncher.remoteFirewall >= FIREWALL.RANDOM || c.puncher.nat.firewall >= FIREWALL.RANDOM) {
    if (c.dht._randomPunches >= c.dht._randomPunchLimit) {
      if (!c.relayToken) throw HOLEPUNCH_ABORTED()
      await c.sleeper.pause(5000 + Math.round(Math.random() * 5000))
      return roundPunch(c, serverAddress, remoteToken, clientRelay)
    }
  }
...
function maybeDestroyEncryptedSocket (c, err) {
  c.session.destroy()
  if (!c.encryptedSocket.rawStream) c.encryptedSocket.destroy(err)
}
...
async function abort (c, { peerAddress, relayAddress }, err) {
  ...
  if (c.relayToken === null) maybeDestroyEncryptedSocket(c, err)
}
