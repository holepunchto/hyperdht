...
async function holepunch (c, opts) {
  ...
  console.log('[client] remoteHolepunchable?', remoteHolepunchable)
  console.log('[client] relayed?', relayed)
  console.log('[client] firewall open?', payload.firewall === FIREWALL.OPEN)
  console.log('[client] relayed and not holepunchable?', (relayed && !remoteHolepunchable))

  if (payload.firewall === FIREWALL.OPEN || (relayed && !remoteHolepunchable)) {
    console.log('[client] method A (firewall open OR relayed + not holepunchable)')

    console.log('remote client address', clientAddress)
    console.log('remote server address', serverAddress)

    const serverAddresses = payload.addresses4 // .filter(onlyPrivateHosts)
    const myAddresses = Holepuncher.localAddresses(c.dht.socket, true)
    const addr = Holepuncher.matchAddress(myAddresses, serverAddresses)

    console.log('my addresses', myAddresses)
    console.log('server addresses', serverAddress)
    console.log('match', addr)

    console.log('choosen', payload.addresses4[0])
  ...
  if (c.lan && relayed && clientAddress.host === serverAddress.host) {
    ...
    if (serverAddresses.length > 0) {
      const myAddresses = Holepuncher.localAddresses(c.dht.io.serverSocket, true)
      const addr = Holepuncher.matchAddress(myAddresses, serverAddresses) || serverAddresses[0]

      console.log('c.serverAddress', c.serverAddress)

      console.log('my addresses', myAddresses)
      console.log('server addresses', serverAddresses)
      console.log('match', addr)

      console.log('choosen', addr)
  ...
  console.log('[client] creating holepuncher ...')
...
async function connectThroughNode (c, address, socket) {
  ...
  if (!c.requesting) {
    ...
    const localAddrs = c.lan ? Holepuncher.localAddresses(c.dht.io.serverSocket, true) : null
    const addresses4 = []

    if (addr) addresses4.push(addr)
    if (localAddrs) addresses4.push(...localAddrs)

    console.log('[client] sharing addresses (with remote? ' + (!!addr) + ')', addresses4)
  ...
  if (isDone(c) || c.connect) {
    console.log('[client] is done (2)', { isDone: isDone(c), connect: !!c.connect })
    return
  }

  const payload = await c.handshake.recv(noise)
  if (isDone(c) || !payload) {
    console.log('[client] is done (3)')
    return
  }
  ...
  if (c.serverSocket) {
    console.log('[client] connectThroughNode (serverSocket)', c.serverAddress)
    c.onsocket(c.serverSocket, c.serverAddress.port, c.serverAddress.host)
    return
  }

  if (!relayed) {
    console.log('[client] connectThroughNode (!relayed)', address)
