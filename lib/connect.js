...
async function holepunch (c, opts) {
  let { relayAddress, serverAddress, payload } = c.connected
  ...
  if (!remoteHolepunchable) {
    if (c.firewalled && payload.firewall === FIREWALL.OPEN) {
      // Remote will connect to us, do nothing as we'll get a connection or timeout
      return
    }

    if (!c.firewalled || payload.firewall === FIREWALL.OPEN) {
      // We told the server we are not firewalled, so we should connect to it.
...
async function connectThroughNode (c, node) {
  if (!c.request) {
    ...
    c.request = c.handshake.send({
      error: 0,
      firewall: c.firewalled ? FIREWALL.UNKNOWN : FIREWALL.OPEN,
