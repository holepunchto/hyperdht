...
const diagnostic = require('./holepunch-diagnostics')
...
async function holepunch(c, opts) {
  ...
  diagnostic('client', 'route-decision', {
    lanEnabled: c.lan,
    relayed,
    sameObservedHost: clientAddress.host === serverAddress.host,
    clientAddress,
    serverAddress,
    relayAddress,
    remoteFirewall: payload.firewall,
    remoteAddresses: payload.addresses4,
    remoteHolepunchable,
    fastOpen
  })
  ...
  if (c.lan && relayed && clientAddress.host === serverAddress.host) {
    ...
    if (serverAddresses.length > 0) {
      ...
      diagnostic('client', 'lan-attempt', {
        localAddresses: myAddresses,
        serverAddresses,
        matchedAddress,
      ...
      function onlanconnect() {
        diagnostic('client', 'lan-ping-result', {
          ok: true,
          address: addr,
          ignored: isDone(c)
        })
  ...
  diagnostic('client', 'probe-start', {
    fastOpen,
    fastOpenAddress: fastOpen ? serverAddress : null
  })
  ...
  try {
    ...
    diagnostic('client', 'probe-error', { error: errorDetails(err) })
    destroyPuncher(c)
  ...
  diagnostic('client', 'probe-result', {
    connected: isDone(c),
    hasProbe: !!probe,
    peerAddress: probe ? probe.peerAddress : null,
    localFirewall: c.puncher ? c.puncher.nat.firewall : null,
    remoteFirewall: c.puncher ? c.puncher.remoteFirewall : null,
    localAddresses: c.puncher ? c.puncher.nat.addresses : null,
    remoteAddresses: c.puncher ? c.puncher.remoteAddresses : null
  })
  ...
  try {
    ...
    diagnostic('client', 'coordinated-punch-error', { error: errorDetails(err) })
    destroyPuncher(c)
...
async function connectThroughNode(c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    ...
    diagnostic('client', 'socket-selected', {
      address: { host, port },
      socketType:
        c.puncher && socket === c.puncher.socket
          ? 'holepunch'
          : socket === c.dht.io.serverSocket
            ? 'dht'
            : 'other'
    })
...
async function updateHolepunch(c, peerAddress, relayAddr, payload) {
  diagnostic('client', 'coordination-send', {
    peerAddress,
    relayAddress: relayAddr,
    round: payload.round,
    firewall: payload.firewall,
    punching: payload.punching,
    addresses: payload.addresses,
    fastOpenAddress: payload.remoteAddress
  })
  ...
  diagnostic('client', 'coordination-receive', {
    peerAddress: holepunch.peerAddress,
    error,
    firewall,
    punching,
    addresses,
    connected: remotePayload.connected,
    round: remotePayload.round
  })
...
async function probeRound(c, serverAddress, serverRelay, retry) {
  ...
  diagnostic('client', 'nat-analysis', {
    stage: 'initial',
    stable,
    sampled: c.puncher.nat.sampled,
    localFirewall: c.puncher.nat.firewall,
    remoteFirewall: c.puncher.remoteFirewall,
    localAddresses: c.puncher.nat.addresses,
    remoteAddresses: c.puncher.remoteAddresses
  })
...
async function roundPunch(c, serverAddress, remoteToken, clientRelay, serverRelay, delayed) {
  ...
  const punching = await c.puncher.punch()
  diagnostic('client', 'coordinated-punch-result', { punching })
  if (!punching) {
...
function maybeDestroyEncryptedSocket(c, err) {
  ...
  diagnostic('client', 'connection-error', { error: errorDetails(err) })
  c.session.destroy()
  c.encryptedSocket.destroy(err)
}

async function abort(c, { peerAddress, relayAddress }, err) {
  diagnostic('client', 'abort-send', {
    error: errorDetails(err),
    peerAddress,
    relayAddress,
    round: c.round,
...
function errorDetails(err) {
  return err ? { code: err.code || null, message: err.message || String(err) } : null
}
