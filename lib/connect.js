...
module.exports = function connect (dht, publicKey, opts = {}) {
  ...
  const c = {
    ...
    puncher: null,
    payload: null,
    receiveConnectTimeout: null,
    clearLocalPings: null,
    serverSocket: null,
    serverAddress: null,
    onsocket: null,
    sleeper: new Sleeper(),
  ...
  encryptedSocket.on('close', function () {
    if (c.receiveConnectTimeout) clearReceiveConnectTimeout(c)
    if (c.clearLocalPings) c.clearLocalPings()
...
async function holepunch (c, opts) {
  ...
  if (payload.firewall === FIREWALL.OPEN || (relayed && !remoteHolepunchable)) {
    if (payload.addresses && payload.addresses.length) {
      const socket = c.dht.io.serverSocket // FIXME: use dht.socket instead when dht-rpc only uses udx!
  ...
  if (c.firewall === FIREWALL.OPEN) {
    c.receiveConnectTimeout = setTimeout(onabort, 10000)
    return
  }

  // TODO: would be better to just try local addrs in the background whilst continuing with other strategies...
  if (!relayed && payload.addresses && clientAddress.host === serverAddress.host && tryLocalAddresses(c, payload.addresses)) {
    return
  }
...
async function connectThroughNode (c, address, socket) {
  ...
  c.onsocket = function (socket, port, host) {
    ...
    if (c.receiveConnectTimeout) {
      clearReceiveConnectTimeout(c)
    }

    if (c.clearLocalPings) {
      c.clearLocalPings()
    }
  }
...
function tryLocalAddresses (c, addresses) {
  ...
  const holder = c.dht._socketPool.acquire()
  let tries = 4

  holder.onholepunchmessage = function (msg, addr) {
    c.onsocket(holder.socket, addr.port, addr.host)
    holder.release()
  }

  send()
  const interval = setInterval(send, 1000)
  c.clearLocalPings = clear

  return true

  function clear () {
    clearInterval(interval)
    c.clearLocalPings = null
    holder.release()
    if (isDone(c)) return
    c.encryptedSocket.destroy(new Error('Local network connection failed'))
  }

  function send () {
    for (const addr of addresses) {
      if (!isPrivate(addr.host)) continue
      Holepuncher.ping(holder.socket, addr)
    }
...
function clearReceiveConnectTimeout (c) {
  clearTimeout(c.receiveConnectTimeout)
  c.receiveConnectTimeout = null
}
