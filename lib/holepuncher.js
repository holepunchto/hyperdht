...
module.exports = class Holepuncher {
  ...
  static findMatchingAddress (myAddresses, externalAddresses) {
    return findMatchingAddress(myAddresses, externalAddresses)
  }
}
...
function findMatchingAddress (myAddresses, externalAddresses) {
  let match = null

  for (const myAddress of myAddresses) {
    // myAddress.host => 192.168.122.238
    const a = myAddress.host.split('.')

    for (const externalAddress of externalAddresses) {
      // externalAddress.host => 192.168.0.23
      // externalAddress.host => 192.168.122.1
      const b = externalAddress.host.split('.')

      // Matches 192.168.*.*
      if (a[0] === b[0] && a[1] === b[1]) {
        if (!match) match = externalAddress

        // Matches 192.168.122.*
        if (a[2] === b[2]) return externalAddress
      }
    }
  }

  return match || externalAddresses[0]
}
