...
module.exports = class Holepuncher {
  ...
  async _start () {
    ...
    if (this.nat.type === 1 && this.remoteNat === 1) {
      ...
      this._keepAliveRandomNat() // run in background
    }
  }
  ...
  async _randomProbes () {
    while (!this.destroyed) {
      ...
      await holepunch(this.socket, addr, DEFAULT_TTL)
  ...
  async _keepAliveRandomNat () {
    ...
    while (!this.destroyed) {
      if (i === this.allSockets.length) {
        i = 0
        if (lowTTLRounds === 0 || --lowTTLRounds <= 0) ttl = DEFAULT_TTL
      }

      await holepunch(this.allSockets[i++], this.remoteAddress, ttl)
  ...
  async _openOtherSockets () {
    while (!this.destroyed && this.allSockets.length < BIRTHDAY_SOCKETS) {
  ...
  destroy () {
    if (this.destroyed) return
    this.destroyed = true
console.log('destroy holepuncher')
    this.nat.destroy()
