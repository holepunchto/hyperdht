...
module.exports = class Holepuncher {
  ...
  punch () {
    if (!this._punching) this._punching = this._punch(false)
  ...
  async _punch (retry) {
    ...
    if (!retry && !this.guard.tryWait()) {
      this.paused = true

      if (this.relayed) {
        this.guard.wait().then(this._retryRelayed.bind(this), this._autoDestroy.bind(this))
        return true
      }
  ...
  async _retryRelayed (destroyed) {
    if (destroyed) return this.destroy()
    this._punching = this._punch(true)
  }
