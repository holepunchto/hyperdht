...
module.exports = class Holepuncher {
  ...
  _findWrap (utp) {
    for (const socket of this._allSockets) {
      if (socket.socket === utp) return socket
    }
  ...
  async reroll () {
    ...
    await this.rerolling
    return this.nat.type === 1
  }

  async _reroll () {
    for (let i = 0; this.nat.type >= 2 && i < MAX_REROLLS && !this.destroyed; i++) {
  ...
  async _consistentProbe () {
    ...
    while (this.punching) {
      ...
      if (this.punching) await this._sleep(1000)
    }
  }
