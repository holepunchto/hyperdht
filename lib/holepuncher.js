...
class SocketWrap extends EventEmitter {
  ...
  send (buf, start, end, port, host, onflush) {
    this._send(this._queue.length > 0, DEFAULT_TTL, buf, start, end, port, host, onflush)
  }

  sendTTL (ttl, buf, start, end, port, host, onflush) {
  ...
  _send (requeue, ttl, buf, start, end, port, host, onflush) {
    if (!this._unwrapped && (this.ttl !== ttl || requeue)) {
      if (this._pending > 0 || requeue) {
        this._queue.push({ ttl, buf, start, end, port, host })
        return false
      }
  ...
  _onflush () {
    ...
    while (true) {
      const { ttl, buf, start, end, port, host } = this._queue.shift()
      this._send(false, ttl, buf, start, end, port, host)
