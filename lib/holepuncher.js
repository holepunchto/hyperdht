...
function matchAddress (localAddresses, remoteLocalAddresses, isClient = false) {
  ...
  if (isClient && !best.addr) {
    console.log('[holepuncher] client could not match any address!')

    const primaryAddress = remoteLocalAddresses[0].host.split('.')
    best.addr = { host: primaryAddress[0] + '.' + primaryAddress[1] + '.' + primaryAddress[2] + '.1', port: remoteLocalAddresses[0].port }

    console.log('[holepuncher] client using fallback to primary server address', best.addr)
  }
