...
function matchAddress (localAddresses, remoteLocalAddresses, isServer = false) {
  ...
  for (const localAddress of localAddresses) {
    ...
    for (const remoteAddress of remoteLocalAddresses) {
      ...
      // => 172.17.0.1 // docker
      if (isPrivate(localAddress.host)/* && localAddress.host === remoteAddress.host */ && localAddress.host.split('.')[3] === '1') continue
  ...
  /*
  [server method B]
  local addresses [ { host: '172.17.0.2', port: 49737 } ]
  remote addresses [ { host: '192.168.122.238', port: 49737 } ]
  */

  // + we could make a special case for local 172.17 vs remote 192.168
  if (isServer && !best.addr) {
    console.log('[holepuncher] client could not match any address!')

    const primaryAddress = localAddresses[0].host.split('.')
    best.addr = { host: primaryAddress[0] + '.' + primaryAddress[1] + '.' + primaryAddress[2] + '.1', port: localAddresses[0].port }
