...
module.exports = class Holepuncher {
  ...
  static localAddresses (socket, b) {
    return localAddresses(socket, b)
  }

  static matchAddress (myAddresses, externalAddresses, third) {
    return matchAddress(myAddresses, externalAddresses, third)
  }
}
...
function localAddresses (socket, isClient = true) {
  const addrs = []
  ...
  for (const n of socket.udx.networkInterfaces()) {
    ...
    // Filter primary addresses
    /* if (isClient && isPrivate(n.host)) {
      const host = n.host.split('.')
      if (host[3] === '1') continue
    } */
...
function matchAddress (localAddresses, remoteLocalAddresses, isServer = false) {
  ...
  for (const localAddress of localAddresses) {
    ...
    for (const remoteAddress of remoteLocalAddresses) {
      // Full match
      // if (localAddress.host === remoteAddress.host) return remoteAddress

      // Skip full matches for private primary addresses
      // => 192.168.122.1 // virtual machines
      // => 172.17.0.1 // docker
      /* if (isPrivate(localAddress.host) && localAddress.host.split('.')[3] === '1') { // && localAddress.host === remoteAddress.host
        console.log('[holepuncher] skipping private primary address', localAddress.host)
        continue
      } */
      ...
      if (a[0] === b[0]) {
        ...
        if (a[1] === b[1]) {
          ...
          // Matches 192.168.122.*
          if (a[2] === b[2]) {
            if (best.segment === 3) best = { segment: 4, addr: remoteAddress }
          }
        }
      }
    }
  ...
  if (!best.addr) { //  && isServer
    console.log('[holepuncher] client could not match any address!')

    // const primaryAddress = remoteLocalAddresses[0].host.split('.')
    // best.addr = { host: primaryAddress[0] + '.' + primaryAddress[1] + '.' + primaryAddress[2] + '.1' }

    // if ('port' in remoteLocalAddresses[0]) best.port = remoteLocalAddresses[0].port // + temp fix for tests

    // best.addr = remoteLocalAddresses[0]

    console.log('[holepuncher] server using fallback to primary server address', best.addr)
  }

  return best.addr || remoteLocalAddresses[0]
}
...
function isSameNetwork (a, b) {
  ...
  a = a.split('.')
  b = b.split('.')
  return a[0] === b[0] && a[1] === b[1] // + or just [0]?
}
