...
class Pair {
  ...
  _connect (addr) {
    ...
    function gc () {
      ...
      self._directConnections[i] = self._directConnections.pop()
    }
  }
  ...
  _unref () {
    ...
    if (pairs.length === 0 || pairs[this._index] !== this) return

    pairs[this._index] = pairs.pop()
    pairs[this._index]._index = this._index
    this._index = 0
  }
}
...
module.exports = class SocketPairer {
  ...
  _addPair (p) {
    for (let i = 0; i < this._incoming.length; i++) {
      ...
      this._incoming[i] = this._incoming.pop()
  ...
  onconnection (rawSocket) {
    ...
    function onclose () {
      ...
      if (i === -1) return
      self._incoming[i] = self._incoming.pop()
    }
