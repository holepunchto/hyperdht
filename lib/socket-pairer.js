...
class Pair {
  constructor (sockets, handshake) {
    ...
    pool.pair(this.handshake.streamId, this._onpair.bind(this))
  }
  ...
  // Note that this never throws so it is safe to run in the background
  async _onholepunchmessage (buf, rinfo, holder) {
    const { socket } = holder
    ...
    while (!this.destroyed && !this.connected) {
      await holepunch(socket, { host: rinfo.host, port: rinfo.port }, false)
  ...
  _makeSocket () {
    ...
    holder.onholepunchmessage = this._onholepunchmessage.bind(this)
...
module.exports = class SocketPairer {
  constructor (dht, socket) {
    ...
    this._socket = dht._socketPool.acquire() // TODO: use main socket
