...
class Pair {
  ...
  open () {
    ...
    this._onpreconnectbound = onpreconnect
    this._reset()

    function onpreconnect (id, addr) {
      if (id !== self.handshake.streamId) return
      const s = self.socket.unwrap()
      const c = udx.createStream(self.handshake.streamId)
      c.connect(s, self.handshake.remoteStreamId, addr.port, addr.host)
      self._sockets.onconnection(c)
    }
  ...
  _onudxconnection (rawSocket, data, ended, from) {
    ...
    if (from) {
      if (this._sockets.addPool(rpub, socket, from)) {
        socket.on('message', (buf, rinfo) => this._sockets.dht.onmessage(socket, buf, rinfo))
      }
    ...
    this.connected = true
    this._shutdown(externalSocket ? null : this._findWrap(socket))
    ...
    function onerror () {
      sockets.gcPool(rpub, socket, true)
      reuse = false
    }

    function onclose () {
      if (sockets.gcPool(rpub, socket, false)) return
  ...
  async _onmessage (socket, buf, rinfo) {
    ...
    if (this.handshake.isInitiator) {
      const s = socket.unwrap()
      const c = udx.createStream(this.handshake.streamId)
      c.connect(s, this.handshake.remoteStreamId, rinfo.port, rinfo.address)
      this._onudxconnection(c, null, false, { id: null, host: rinfo.address, port: rinfo.port })
      return
    }
  ...
  _findWrap (socket) {
    for (const wrap of this._allSockets) {
      if (wrap.socket === socket) return wrap
    }
  ...
  _makeSocket () {
    ...
    socket.bind()
    socket.on('preconnect', this._onpreconnectbound)
  ...
  connectUDX (addr, socket) {
    if (!socket) {
      socket = udx.createSocket()
      socket.bind()
    }

    const rawSocket = udx.createStream(this.handshake.streamId)
    rawSocket.connect(socket, this.handshake.remoteStreamId, addr.port, addr.host)

    this._onudxconnection(rawSocket, null, false, null)
  }
  ...
  _connect (addr) {
    ...
    function connect () {
      ...
      self._onudxconnection(rawSocket, null, false, null)
    }

    function close () {
      // TODO: when not deferred, the socket is closed before the other side
      // has its `end` message ack'ed; how can we handle this?
...
module.exports = class SocketPairer {
  constructor (dht, socket) {
    ...
    this._socket = socket
    this._socket.on('preconnect', this.onpreconnect.bind(this))
  ...
  _notify (inc) {
    for (let i = 0; i < this._pairs.length; i++) {
      ...
      p._onudxconnection(rawSocket, data, ended, null)
      return
    }
  ...
  onpreconnect (id, addr) {
    const pair = this._pairs.find((pair) => pair.handshake.streamId === id)
    ...
    // TODO: this errors when the other side closes, figure out why
    const rawSocket = udx.createStream(pair.handshake.streamId)
    rawSocket.connect(socket, pair.handshake.remoteStreamId, addr.port, addr.host)
  ...
  addPool (publicKey, socket, address) {
    ...
    if (pool) {
      if (pool.socket === socket) {
        if (pool.timeout && pool.socket.streams.size > 0) {
  ...
  gcPool (publicKey, socket, now) {
    ...
    if (pool.socket.streams.size > 0) return true
...
function removePool (pool, map, id) {
  ...
  pool.timeout = null
  if (pool.socket.streams.size > 0) return
  map.delete(id)
  pool.socket.close()
}
