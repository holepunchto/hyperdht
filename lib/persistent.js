...
module.exports = class PersistentNode {
  ...
  onmutableget (req) {
    ...
    const localSeq = cenc.decode(cenc.uint, value)
  ...
  onmutableput (req) {
    ...
    const store = this.mutables
    const { seq, value, signature, publicKey } = cenc.decode(messages.mutablePutRequest, req.value)
    ...
    store.set(k, cenc.encode(messages.mutableGetResponse, { seq, value, signature }))
  ...
  static signMutable (seq, value, secretKey) {
    ...
    sodium.crypto_generichash(signable, cenc.encode(messages.signable, { seq, value }), NS_MUTABLE)
