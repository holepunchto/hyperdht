...
const { isLeaf } = require('prefix-hash-tree/node')
...
module.exports = class Persistent {
  constructor(dht, opts) {
    ...
    this.authenticatedPHTNodes = new Cache(opts.records)
    this.phtShards = new RecordCache(opts.records)
  }
  ...
  // TODO: does this require any additional sophistication?
  onauthenticatedphtnodeget(req) {
    if (!req.target) return

    const k = b4a.toString(req.target, 'hex')
    const phtGetResponse = this.authenticatedPHTNodes.get(k)

    req.reply(phtGetResponse || null)
  }

  async onauthenticatedphtnodeput(req) {
    if (!req.target || !req.token || !req.value) return

    // TODO: authentication!

    const unslabbed = unslab(req.value)
    const p = decode(m.authenticatedPHTNodePutRequest, unslabbed)
    if (p === null) return

    const { _publicKey, _topologyID, phtNode, _signature } = p 

    // TODO: must we store the whole authenticatedPHTNodePutRequest, or can we discard some of it?
    const k = b4a.toString(req.target, 'hex')
    this.authenticatedPHTNodes.set(k, unslabbed)

    // TODO: this is the wrong layer of abstraction to do this, because we're announcing shard
    // replication but the server is implemented at the hypersearch layer...
    if (isLeaf(phtNode)) {
      const stream = this.dht.announce(req.target, this.dht.defaultKeyPair)
  ...
  onphtshardget(req) {
    ...
    // TODO: get and decode the stored PHT node and validate the PHT key against it

    const k = b4a.concat([req.target, req.value]).toString('hex')

    // TODO: parameterize get qty?
    const docs = this.phtShards.get(k, 1000)
    const encoded = c.encode(m.phtShardGetResponse, { records: docs })
  ...
  onphtshardput(req) {
    ...
    const { key, value } = c.decode(m.phtShardPutRequest, unslab(req.value))
    const k = b4a.concat([req.target, key]).toString('hex')
    this.phtShards.add(k, value)
  ...
  destroy() {
    ...
    this.authenticatedPHTNodes.destroy()
  }
  ...
  static signAuthenticatedPHTNode(phtNode, topologyID, keyPair) {
    const signable = b4a.allocUnsafe(32 + 32)
    const hash = signable.subarray(32)

    signable.set(NS.AUTHENTICATED_PHT_NODE_PUT, 0)

    sodium.crypto_generichash(hash, c.encode(m.authenticatedPHTNodeSignable, { topologyID, phtNode }))
    return sign(signable, keyPair)
  }
}
