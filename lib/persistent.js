...
module.exports = class PersistentNode {
  ...
  onget (req, { mutable }) {
    ...
    const { salt, seq } = cenc.decode(messages.mutableReq, req.value)
    const k = salt ? req.target.toString('hex') + salt.toString('hex') : req.target.toString('hex')
    const value = store.get(k)
    if (!value) return req.reply(Buffer.from('')) // empty response so initial puts can still find nodes
    const local = cenc.decode(messages.mutableRes, value)
  ...
  onput (req, { mutable }) {
    ...
    const { seq, value, signature, salt, publicKey } = cenc.decode(messages.mutableRes, req.value)
    if (!value || !signature) return req.reply(null)
    const key = salt
      ? req.target.toString('hex') + salt.toString('hex')
      : req.target.toString('hex')
