...
module.exports = class Persistent {
  ...
  onlookup(req) {
    ...
    req.reply(
      records.length
        ? c.encode(m.lookupRawReply, { peers: records, bump })
        : null
    )
  }
  ...
  onunannounce(req) {
    ...
    if (
      !sodium.crypto_sign_verify_detached(signature, signable, peer.publicKey)
    ) {
      return
    }
  ...
  onannounce(req) {
    ...
    if (announceSelf) {
      ...
      if (bump > currentBump && bump <= Date.now() + MAX_BUMP_DRIFT)
        this.bumps.set(k, bump)
  ...
  onmutableput(req) {
    ...
    this.mutables.set(
      k,
      encodeUnslab(m.mutableGetResponse, { seq, value, signature })
    )
    req.reply(null)
  }
