...
const { isNode } = require('prefix-hash-tree/node')
...
module.exports = class Persistent {
  ...
  onauthenticatedphtnodeput(req) {
    ...
    if (!Persistent.verifyAuthenticatedPHTNode(signature, treeID, phtNode, publicKey)) return

    const k = b4a.toString(req.target, 'hex')
    this.authenticatedPHTNodes.set(k, { publicKey, treeID, phtNode, signature })
  ...
  static signAuthenticatedPHTNode(phtNode, treeID, keyPair) {
    const signable = b4a.allocUnsafe(32 + 32)
    const hash = signable.subarray(32)

    signable.set(NS.AUTHENTICATED_PHT_NODE_PUT, 0)

    sodium.crypto_generichash(hash, c.encode(m.authenticatedPHTNodeSignable, { treeID, phtNode }))
    return sign(signable, keyPair)
  }

  static verifyAuthenticatedPHTNode(signature, treeID, phtNode, publicKey) {
    const signable = b4a.allocUnsafe(32 + 32)
    const hash = signable.subarray(32)
