...
module.exports = class Persistent {
  constructor(dht, opts) {
    ...
    this.searchableRecords =
      opts.experimentalSearch === true ? new SearchableRecordCache(opts.searchableRecords) : null
  }
  ...
  onsearch(req) {
    if (!req.target || !req.value || !this.searchableRecords) {
  ...
  destroy() {
    ...
    if (this.searchableRecords) this.searchableRecords.destroy()
  }
