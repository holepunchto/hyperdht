...
module.exports = class Persistent {
  ...
  onauthenticatedphtnodeget(req) {
    ...
    const k = b4a.toString(req.target, 'hex')
    const p = this.authenticatedPHTNodes.get(k)

    if (p === null) return req.reply(null)
    
    const phtGetResponse = c.encode(m.authenticatedPHTNodeGetResponse, p)
  ...
  onauthenticatedphtnodeput(req) {
    ...
    const { publicKey, topologyID, phtNode, signature, connectionKey } = p 

    // TODO: authentication!

    const k = b4a.toString(req.target, 'hex')
    this.authenticatedPHTNodes.set(k, { publicKey, topologyID, phtNode, signature })
