...
module.exports = class Announcer {
  ...
  async _update () {
    ...
    console.log('_update', b4a.toString(this.target, 'hex'), { hash: false, nodes: this._closestNodes, infligth: q.inflight })
    const logInterval = setInterval(() => console.log(`_update ${b4a.toString(this.target, 'hex')} inflight=${q.inflight}`), 500)
    logInterval.unref()
    try {
      await q.finished()
    } catch {
