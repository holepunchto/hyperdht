...
module.exports = class Announcer {
  constructor(dht, keyPair, target, opts = {}) {
    ...
    this.record = encodeUnslab(m.peer, {
  ...
  async _unannounceCurrent() {
    ...
    const un = (this._unannouncing = this._unannounceAll(
      this._serverRelays[2].values()
    ))
  ...
  async _background() {
    while (!this.dht.destroyed && !this.stopped) {
      try {
        ...
        for (
          let i = 0;
          i < 100 && !this.stopped && !this._refreshing && !this.suspended;
          i++
        ) {
          const pings = []
