...
exports.noisePayload = {
  preencode (state, m) {
    ...
    if (m.invalidateStream >= 0) c.uint.preencode(state, m.invalidateStream)
  },
  encode (state, m) {
    let flags = 0
    ...
    if (m.invalidateStream >= 0) flags |= 64
    ...
    if (m.invalidateStream >= 0) c.uint.encode(state, m.invalidateStream)
  },
  decode (state) {
    const version = c.uint.decode(state)
    ...
    return {
      ...
      invalidateStream: (flags & 64) !== 0 ? c.uint.decode(state) : -1
    }
  }
}
