...
exports.noisePayload = {
  preencode (state, m) {
    ...
    if (m.invalidateStream) c.fixed32.preencode(state, m.invalidateStream)
  },
  encode (state, m) {
    let flags = 0
    ...
    if (m.invalidateStream) flags |= 64
    ...
    if (m.invalidateStream) c.fixed32.encode(state, m.invalidateStream)
  },
  decode (state) {
    const version = c.uint.decode(state)
    ...
    return {
      ...
      invalidateStream: (flags & 64) !== 0 ? c.fixed32.decode(state) : null
    }
  }
}
