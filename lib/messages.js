...
exports.noisePayload = {
  preencode (state, m) {
    state.end += 5 // version + flags + error + network + protocols
  ...
  decode (state) {
    ...
    if (version !== 1) throw new Error('Version mismatch')

    const flags = c.uint.decode(state)

    return {
      error: c.uint.decode(state),
      network: c.uint.decode(state),
...
exports.holepunchPayload = {
  preencode (state, m) {
    state.end += 3 // flags + error + network
