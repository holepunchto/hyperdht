...
const SynapticWeight = require('./synaptic-weight')
...
module.exports = class ConnectionPool extends EventEmitter {
  constructor(dht) {
    ...
    this._synaptic = new SynapticWeight()
  }
  ...
  updateWeightSuccess(publicKey, latency, bandwidth) {
    const keyString = b4a.toString(publicKey, 'hex')
    const ref = this._connections.get(keyString) || this._connecting.get(keyString)
    if (!ref) return
    ref.latency = latency
    ref.bandwidth = bandwidth
    ref.weight = this._synaptic.updateOnSuccess(ref.weight, latency, bandwidth)
  }

  updateWeightFailure(publicKey) {
    const keyString = b4a.toString(publicKey, 'hex')
    const ref = this._connections.get(keyString) || this._connecting.get(keyString)
    if (!ref) return
    ref.weight = this._synaptic.updateOnFailure(ref.weight)
  }
  ...
  applyBackpressure(publicKey, isOverloaded) {
    ...
    const ref = this._connections.get(keyString) || this._connecting.get(keyString)
    if (!ref) return
    ref.isOverloaded = isOverloaded
    ref.weight = this._synaptic.updateOnBackpressure(ref.weight, isOverloaded)
  }

  getPeerWeights() {
    const result = []
    for (const [key, ref] of this._connections) {
      result.push({ publicKey: key, weight: ref.weight, isOverloaded: ref.isOverloaded })
    }
    return result
  }
}
