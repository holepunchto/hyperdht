...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    ...
    if (this._shareLocalAddress) {
      for (const addr of this.dht.io.networkInterfaces) {
        if (addr.internal) continue

        if (addr.family === 4) {
          if (addr.host.startsWith('169.254')) continue
          addresses4.push({ host: addr.host, port: this.dht.port })
        } else {
          if (addr.host.startsWith('fe80')) continue
          addresses6.push({ host: addr.host, port: this.dht.port })
        }
      }
    }
