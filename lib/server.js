...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake(k, noise, clientAddress, { to: serverAddress, socket }, direct) {
    ...
    diagnostic('server', 'handshake', {
      direct,
      clientAddress,
      serverAddress,
      sameObservedHost: clientAddress.host === serverAddress.host,
      remoteFirewall: remotePayload.firewall,
      remoteAddresses: remotePayload.addresses4,
      localRemoteAddress: ourRemoteAddr,
      localAddresses: ourLocalAddrs,
      shareLocalAddress: this._shareLocalAddress
    })
    ...
    if (error === ERROR.NONE) {
      ...
      hs.onsocket = (socket, port, host) => {
        ...
        diagnostic('server', 'socket-selected', {
          address: { host, port },
          socketType:
            hs.puncher && socket === hs.puncher.socket
              ? 'holepunch'
              : socket === this.dht.io.serverSocket
                ? 'dht'
                : 'other'
        })
    ...
    const onabort = () => {
      diagnostic('server', 'connection-abort', {
        hasRawStream: !!hs.rawStream,
        relayPaired: hs.relayPaired,
        relayToken: !!hs.relayToken
      })
      hs.aborted = true
    ...
    if (!direct && clientAddress.host === serverAddress.host) {
      ...
      if (clientAddresses.length > 0 && this._shareLocalAddress) {
        ...
        diagnostic('server', 'lan-prepunch-decision', {
          clientAddresses,
  ...
  async _onpeerholepunch({ id, peerAddress, payload }, req) {
    ...
    diagnostic('server', 'coordination-receive', {
      peerAddress,
      ...
      punching,
      addresses,
      fastOpenAddress: remoteAddress,
      connectionSelected: h.rawStream === null
    })
    ...
    diagnostic('server', 'nat-analysis', {
      stage: 'initial',
      stable,
      sampled: p.nat.sampled,
      localFirewall: p.nat.firewall,
      remoteFirewall: p.remoteFirewall,
      localAddresses: p.nat.addresses,
    ...
    const fastOpen =
      isConsistent(p.nat.firewall) && remoteAddress && hasSameAddr(p.nat.addresses, remoteAddress)

    diagnostic('server', 'fast-open-decision', {
      enabled: !!fastOpen,
      peerAddress,
      localFirewall: p.nat.firewall,
      localAddresses: p.nat.addresses,
    ...
    if (fastOpen) {
      diagnostic('server', 'fast-open-ping', {
        peerAddress,
        localFirewall: p.nat.firewall,
        localAddresses: p.nat.addresses,
        fastOpenAddress: remoteAddress
    ...
    if (p.remoteHolepunching) {
      diagnostic('server', 'coordinated-punch-start', {
        localFirewall: p.nat.firewall,
        remoteFirewall: p.remoteFirewall,
        localAddresses: p.nat.addresses,
        remoteAddresses: p.remoteAddresses
      ...
      if (p.destroyed) return null
      diagnostic('server', 'coordinated-punch-result', { punching })
  ...
  _abort(h, error = ERROR.ABORTED) {
    diagnostic('server', 'abort-send', {
      error,
      round: h.round,
      localFirewall: h.puncher ? h.puncher.nat.firewall : null,
      remoteFirewall: h.puncher ? h.puncher.remoteFirewall : null
    })
