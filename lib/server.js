...
module.exports = class Server extends EventEmitter {
      ...
      if (p.remoteHolepunching) {
        ...
        if (p.remoteFirewall >= FIREWALL.RANDOM || p.nat.firewall >= FIREWALL.RANDOM) {
          if (
            ...
            if (!h.tryLaterCounted) {
              h.tryLaterCounted = true
              this.dht.stats.punches.tryLaterRelayedHandshakesAtLeastOnce++
            }
            return {
              socket: p.socket,
  ...
  _relayConnection(hs, relayThrough, remotePayload, h) {
    ...
    const dht = this.dht
    function onabort() {
      if (hs.relayClient !== relayClient) return
      // A relay the direct path already made redundant is not an abort
      if (!hs.relayPaired && hs.rawStream !== null) dht.stats.relaying.aborts++
...
function maybeDestroyRelayHandshake(hs, relayRecoveryWait) {
  ...
  // An active holepunch workflow owns cleanup.
