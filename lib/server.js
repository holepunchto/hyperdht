...
module.exports = class Server extends EventEmitter {
  ...
  _localAddresses() {
    return this.dht.validateLocalAddresses(
      Holepuncher.localAddresses(this.dht.io.serverSocket)
    )
  }
  ...
  async _addHandshake(
    ...
    const ourLocalAddrs = this._shareLocalAddress
    ...
    if (error === ERROR.NONE) {
      ...
      hs.onsocket = (socket, port, host) => {
        ...
        if (hs.rawStream.connected) {
          const remoteChanging = hs.rawStream.changeRemote(
            socket,
            remotePayload.udx.id,
            port,
            host
          )
    ...
    hs.puncher = new Holepuncher(
      this.dht,
      this.dht.session(),
      false,
      remotePayload.firewall
    )
  ...
  async _onpeerholepunch({ id, peerAddress, payload }, req) {
    ...
    if (p.remoteHolepunching) {
      ...
      if (
        p.remoteFirewall >= FIREWALL.RANDOM ||
        p.nat.firewall >= FIREWALL.RANDOM
      ) {
        if (
  ...
  _relayConnection(hs, relayThrough, remotePayload, h) {
    ...
    hs.relayClient = relay.Client.from(hs.relaySocket, {
      id: hs.relaySocket.publicKey
    })
