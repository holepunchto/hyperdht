...
module.exports = class Server extends EventEmitter {
  ...
  async _onholepunch ({ id, peerAddress, payload }, req) {
    ...
    if (remotePayload.version !== 1) return abort(h, ERROR.VERSION_MISMATCH)
    ...
    if (error !== ERROR.NONE) return abort(h)
    ...
    if (!punching && p.remoteFirewall >= FIREWALL.RANDOM && p.nat.firewall >= FIREWALL.RANDOM) {
      if (!(await p.reopen())) {
        return p.destroyed ? null : abort(h)
      }
    }
...
function abort (h, error = ERROR.ABORTED) {
  if (!h.pair.payload) {
    h.pair.destroy()
    return null
  }
