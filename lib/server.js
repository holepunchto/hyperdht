...
module.exports = class Server extends EventEmitter {
  ...
  async _onconnect ({ noise, peerAddress }, req) {
  ...
  async _onholepunch ({ id, peerAddress, payload }, req) {
    ...
    if ((p.remoteAddress === null || punching) && firewall !== FIREWALL.UNKNOWN && address) {
      if (p.remoteAddress && p.remoteAddress.host !== address.host) p.remoteVerified = false
    ...
    if (p.remoteHolepunching && p.remoteVerified) {
      ...
      if (!this.holepunch(p.remoteFirewall, p.nat.firewall, p.remoteAddress, p.nat.address)) {
