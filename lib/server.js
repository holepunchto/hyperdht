...
class ServerConnector {
  ...
  async _addHandshake (k, noise, peerAddress) {
    ...
    const h = {
      isInitiator: false,
      id: NoiseSecretStream.id(handshake.hash),
      publicKey: toBuffer(this.server.keyPair.publicKey),
      remotePublicKey: toBuffer(handshake.rs),
      tx: toBuffer(handshake.tx),
      rx: toBuffer(handshake.rx),
      hash: toBuffer(handshake.hash)
    }

    const fw = this.server.firewall(handshake.rs, remotePayload, peerAddress)
    const hs = {
      reply,
      pair: null,
    ...
    pair.onconnection = (rawSocket, data, ended) => {
      this._clear(hs, id, k)
    ...
    if (!this.server.dht.firewalled && false) {
      return hs
    }
...
function abort (h) {
  const payload = h.pair.encryptPayload({
    status: ABORT,
    nat: 0,
    address: null,
    remoteAddress: null,
...
function toTrue () { // TODO: safety catch this?
  return true
}
