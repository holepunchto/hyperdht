...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    console.log('_addHandshake', { clientAddress, serverAddress })
    ...
    const ourLocalAddrs = this._shareLocalAddress ? Holepuncher.localAddresses(this.dht.io.serverSocket, false) : null

    if (ourRemoteAddr) addresses.push(ourRemoteAddr)
    if (ourLocalAddrs) addresses.push(...ourLocalAddrs)

    console.log('[server] sharing addresses (with remote? ' + (!!ourRemoteAddr) + ')', addresses)
    ...
    console.log('hs', hs)
    console.log('hs.reply', hs.reply) */

    console.log('[server] firewall open?', remotePayload.firewall === FIREWALL.OPEN)
    console.log('[server] direct?', direct)

    if (remotePayload.firewall === FIREWALL.OPEN || direct) {
      console.log('[server] method A (firewall open OR direct)')
      ...
      if (clientAddresses.length > 0) {
        const myAddresses = Holepuncher.localAddresses(direct ? socket : this.dht.socket, false)
        const addr = Holepuncher.matchAddress(myAddresses, clientAddresses)

        console.log('my addresses', myAddresses)
        console.log('client addresses', clientAddresses)
        console.log('match', addr)
      }

      console.log('choosen', clientAddress)
    ...
    if (!direct && clientAddress.host === serverAddress.host) {
      ...
      if (clientAddresses.length > 0) {
        const myAddresses = Holepuncher.localAddresses(this.dht.io.serverSocket, false)
        const addr = Holepuncher.matchAddress(myAddresses, clientAddresses, true)
        ...
        if (addr) {
          ...
          console.log('[server] following with normal remote holepunching')
        }
      }
    }
  ...
  async _onpeerhandshake ({ noise, peerAddress }, req) {
    console.log('_onpeerhandshake', { peerAddress })
  ...
  async _onpeerholepunch ({ id, peerAddress, payload }, req) {
    console.log('_onpeerholepunch', { peerAddress })
