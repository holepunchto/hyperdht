  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    ...
    try {
      hs.reply = await handshake.send({
        ...
        secretStream: {},
        relayThrough: relayThrough && { publicKey: relayThrough, token: hs.relayToken }
      })
    } catch (err) {
      safetyCatch(err)
    ...
    if (relayThrough || remotePayload.relayThrough) {
      ...
      const relaySocket = this.dht.connect(publicKey, { pool: this.pool })
      ...
      const request = relay.Client
        ...
        .on('data', (remoteId) => {
          ...
          hs.rawStream.connect(socket, remoteId, remotePort, remoteHost)

          hs.onsocket(socket, remotePort, remoteHost)
        })
