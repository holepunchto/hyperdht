...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    ...
    const ourLocalAddr6 = this._shareLocalAddress ? this.dht.localAddress({ family: 6 }) : null
    const ourRemoteAddr = this.dht.remoteAddress()

    if (ourRemoteAddr) addresses4.push(ourRemoteAddr)
    if (ourLocalAddr) addresses4.push(ourLocalAddr) // for now always share local addrs, in the future we can do some filtering
    if (ourLocalAddr6) addresses6.push(ourLocalAddr6)
