...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, peerAddress) {
    ...
    hs.reply = handshake.send({
      error: 0,
      network: this.dht.firewalled ? UNKNOWN : OPEN,
      protocols: TCP | UTP,
      holepunch: !this.dht.firewalled ? null : {
        id,
  ...
  async _onholepunch ({ id, peerAddress, payload }, req) {
    ...
    const { error, network, punching, address, remoteAddress, remoteToken } = remotePayload

    if (error > 0) return abort(h)
    ...
    if (p.remoteNat === 0 && network !== 0 && address && (p.remoteNat !== 2 || address.port !== 0)) {
      p.remoteNat = network
    ...
    if (p.nat.type === 2 && remoteAddress && sameAddr(p.nat.address, remoteAddress)) {
