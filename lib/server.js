...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    ...
    // TODO: This object leaks, in both holepunches and connects
    const hs = {
      round: 0,
      reply: null,
      puncher: null,
    ...
    if (error === ERROR.NONE) {
      hs.rawStream = this.dht.createRawStream({
        ...
        firewall (socket, port, host) {
          console.log('firewall check', hs.relaySocket && isRelay(hs.relaySocket, socket, port, host), port, host)
          // TODO: what if we always keep relaying? Then onsocket is never called
          ...
          // TODO: this never runs, otherwise onsocket would run (unless rawStream already null)
      ...
      // TODO: This will fix it
      // Handles the case where onsocket is never called, but data is flowing
      // Which can happen on a relayed connection which never stops relaying
      // (onsocket is called there only when the connection is established)
      hs.rawStream.on('close', () => this._clearLater(hs, id, k))

      hs.onsocket = (socket, port, host) => {
        // console.trace('onsocket called')
        // TODO: this is the happy path for calling _clearLater, so must fail here
    ...
    // Can't be here, because this has _clearLater
    if (error !== ERROR.NONE) {
      hs.rawStream.destroy()
      this._clearLater(hs, id, k)
      return hs
    ...
    if (remotePayload.firewall === FIREWALL.OPEN || direct) {
      ...
      // Can't be here, because this is before relaying
      hs.onsocket(sock, clientAddress.port, clientAddress.host)
      return hs
    }

    // This path happens. ANd it must hit a path where _clearLater is not called
    ...
    // This never runs, because hs.puncher === null
  ...
  _relayConnection (hs, relayThrough, remotePayload, h) {
    ...
    // TODO: this should clean up all attempts after 15s
    hs.relayTimeout = setTimeout(onabort, 15000)

    // relayPaired is false, so it either never reaches 'data', or rawStream is null when it does
    // rawStream is null
    // relayToken is null
    // relaySocket is null
    hs.relayClient
      ...
      .on('data', (remoteId) => {
        ...
        // This definitely does run (relayPaired === true)
        hs.relayPaired = true
        ...
        // Closed check? The rawStream is closed in the heapdump
        // But the relaySocket also seems closed...
        // Note: this is not the bug (the relaySocket ends up destroyed)
        if (hs.rawStream.closed) { // or closing?
          hs.relaySocket.destroy()
          return
        }

        // The relaySocket is for sure destroyed, because
        // it no longer has the keepAlive we defined above
        // (rmeoving that happens in destroy)
        hs.rawStream
          .on('close', () => hs.relaySocket.destroy())
          .connect(socket, remoteId, remotePort, remoteHost)

        // This is created, and never cleaned up, while the onabort cleans the rawStream
        // TODO: this is missing a keepAlive. Is that the bug? (we do set it in the other one)
        hs.encryptedSocket = this.createSecretStream(false, hs.rawStream, { handshake: h, keepAlive: this.dht.connectionKeepAlive })

        this.onconnection(hs.encryptedSocket)
      })

    function onabort () {
      // This definitely does not run (relaySocket still set)
      // relaySocket._openedDOne is still null, so it never opened either
      if (hs.relayTimeout) clearRelayTimeout(hs)
