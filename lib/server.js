...
class ServerConnector {
  constructor (server) {
    this.server = server
    this.relays = server.relays
    this.announcer = server.relayNodes
  ...
  async _addHandshake (k, noise) {
    ...
    const relays = this.relays.slice(0) // copy for now
...
if (token && remoteToken && !echoed) {
  console.log('not echoed', token, remoteToken, echoed, isServerRelay, peerAddress)
    ...
    if (p.remoteNat >= 2 && p.nat.type >= 2) {
      if (!(await p.reroll())) {
console.log('abort due to bad reroll')
        return abort(h)
      }
    }
      ...
      async commit (msg, dht) {
        ...
        if (res.error === 0) {
          ...
          if (picked++ < 3) {
            ...
            if (relays.length > 3) relays.shift()
          }
        }
...
const nodes = q.closestNodes.map(n => n.host + ':' + n.port).sort()
console.log('announces to', hash(Buffer.from(nodes)).toString('hex'))
