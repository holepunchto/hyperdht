...
module.exports = class Server extends EventEmitter {
  ...
  async _onpeerholepunch({ id, peerAddress, payload }, req) {
    ...
    try {
      return await this._onholepunchround(h, peerAddress, payload, req)
    } catch (err) {
      // The router swallows these rejections, which would leak the puncher,
      // its sockets and its random-punch quota. Abort so both sides stop.
      safetyCatch(err)
      this.dht.stats.punches.failed++
      return !h.puncher || h.puncher.destroyed ? null : this._abort(h)
    }
  }

  async _onholepunchround(h, peerAddress, payload, req) {
