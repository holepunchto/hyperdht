...
module.exports = class Server extends EventEmitter {
  ...
  async _addHandshake (k, noise, clientAddress, { from, to: serverAddress, socket }, direct) {
    ...
    if (remotePayload.invalidateStream) {
      console.log('remotePayload.invalidateStream', remotePayload.invalidateStream)
    }
    ...
    if (error === ERROR.NONE) {
      ...
      hs.onsocket = (socket, port, host) => {
        ...
        if (hs.rawStream.connected) {
          ...
          const extra = remotePayload.invalidateStream ? { invalidateStream: remotePayload.invalidateStream } : null
          this.onconnection(hs.encryptedSocket, extra)
        }
    ...
    try {
      console.log('sending handshake (server)')
      ...
      console.log('received handshake reply (server)')
    } catch (err) {
      safetyCatch(err)
