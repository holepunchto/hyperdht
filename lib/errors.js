module.exports = class DHTError extends Error {
  ...
  static HOLEPUNCH_PROBE_TIMEOUT(
    msg = 'Holepunching probe did not finish in time'
  ) {
    return new DHTError(
      msg,
      'HOLEPUNCH_PROBE_TIMEOUT',
      DHTError.HOLEPUNCH_PROBE_TIMEOUT
    )
  }

  static HOLEPUNCH_DOUBLE_RANDOMIZED_NATS(
    msg = 'Both remote and local NATs are randomized'
  ) {
    return new DHTError(
      msg,
  ...
  static REMOTE_NOT_HOLEPUNCHING(msg = 'Remote is not holepunching') {
    return new DHTError(
      ...
      DHTError.REMOTE_NOT_HOLEPUNCHING
    )
  }
  ...
  static REMOTE_NOT_HOLEPUNCHABLE(msg = 'Remote is not holepunchable') {
    return new DHTError(
      ...
      DHTError.REMOTE_NOT_HOLEPUNCHABLE
    )
  }
  ...
  static HANDSHAKE_UNFINISHED(msg = 'Handshake did not finish') {
    return new DHTError(
      msg,
      'HANDSHAKE_UNFINISHED',
      DHTError.HANDSHAKE_UNFINISHED
    )
  }
  ...
  static PEER_CONNECTION_FAILED(msg = 'Could not connect to peer') {
    return new DHTError(
      ...
      DHTError.PEER_CONNECTION_FAILED
    )
  }
