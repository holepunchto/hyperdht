...
test('announce & lookup with data length updated', async ({ is }) => {
  const { bootstrap, closeDht, port: bsPort } = await dhtBootstrap()
  const peer = dht({
    bootstrap,
    ephemeral: true
  ...
  await peer.announce(topic, { port, length: 43 })

  ;[{ node, peers, localPeers }] = await peer.lookup(topic, { includeLength: true })
  ...
  const res = await peer.lookup(topic, { includeLength: true })
