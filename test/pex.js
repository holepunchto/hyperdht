...
test('peer exchange', async (t) => {
  ...
  const as = a.createServer({ reusableSocket: true })
  await as.listen()

  t.comment('a key', as.publicKey.toString('hex'))

  const bs = b.createServer({ reusableSocket: true })
  await bs.listen()
...

  {
    ...
    b.connect(as.publicKey, { reusableSocket: true })
      .on('error', () => {})
      .on('open', () => {
...

  {
    ...
    const stream = c.connect(bs.publicKey, { reusableSocket: true })
      .on('error', () => {})
      .on('open', async () => {
